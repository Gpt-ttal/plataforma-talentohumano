...
export function cerrarRegistroCurso(deps: { repo: CursoRepo }) {
  return async (actor: Usuario, id: string): Promise<Curso> => {
    ...
    if (detalle.curso.estadoRegistro !== "ABIERTO") {
      throw new ErrorValidacion("Solo se puede cerrar el registro cuando está ABIERTO.")
    }

    return deps.repo.cambiarEstadoRegistro(id, "CERRADO", "ABIERTO")
  }
}
